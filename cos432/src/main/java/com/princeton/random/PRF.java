...
/*            prf.update(content);                                                */
/*        There is another overload of update() that allows you to specify a      */
/*        slice of the input that you wish to add to the buffer.                  */
/*                                                                                */
/*        All overloads of the eval() function take an input, append it to the    */
/*        buffer that the PRF object maintains, and then evaluate the             */
/*        pseudo-random function on the contents of the buffer (with the          */
/*        appended input included). A call to eval() resets the buffer.           */
/*        For example:                                                            */
/*            byte[] out = PRF.eval(newContent);                                  */
/*        <out> is the result of calling the pseudo-random function with key k    */
/*        on the concatenation of the current buffer (content provided via        */
/*        previous calls to update() since the last call to eval()) and           */
/*        newContent.                                                             */
/*        <out> is of length OUTPUT_SIZE_BYTES.                                   */
/*                                                                                */
/*        NOTE: You should not need to use the update() function for Assignment   */
/*        1 or 2.  You only might need it for Assignment 3.                       */
/*                                                                                */
/* ------------------------------------------------------------------------------ */
